...
if (resolve('/public/evento/inscricao')) {
...
} else if(resolve('/public/evento/inscricao/create')) {
     if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $create('inscritos');
        return header('location:/public/evento/inscricao');
    }
    $cursos = $findDinamic('curso');
    
    render('evento/inscricao/create', 'evento', ['cursos' => $cursos]);

} else if ($params = resolve('/public/evento/inscricao/(\d+)')) {
...
} else if ($params = resolve('/public/evento/inscricao/(\d+)/edit')) {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $edit('inscritos',$params[1]);
        return header('location:/public/evento/inscricao/'.$params[1]);
    }
    $inscrito = $find('inscritos', 'inscritos.created,inscritos.modified', 'INNER JOIN curso ON inscritos.idcurso = curso.idcurso', 'inscrito_id', $params[1]);
    $cursos = $findDinamic('curso');
...
} else if($params = resolve('/public/evento/inscricao/(\d+)/delete')) {
